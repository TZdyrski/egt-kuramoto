...
Tripp, Fu, and Pauls~\citep{tripp2022evolutionary}
(abbreviated \tripp{} hereafter)
generalized this approach to dynamically changing game types
with their evolutionary Kuramoto (EK) model.
\tripp{} considered a well-mixed player population
characterized by a communicative/non-communicative strategy
and quantized Kuramoto phase.
They showed how the relationship between communication benefit and cost
...
with only brief excursions to deadlock-type games (light blue).
...
it displayed chicken (denoted ``snowdrift'' in \tripp{})
...
by extending the work of \tripp{} to account for benefit asymmetry $\alpha$.
...
For instance, we find a dilemma game type (``prisoner's dilemma'')
if the entries of the payoff matrix \cref{eq:payoff-matrix}
have the following order (with $4$ being the largest):
\begin{equation}
\begin{bNiceMatrix}[first-col,first-row,hvlines]
  & C & N \\
  C & 3 & 1 \\
...
\end{bNiceMatrix}
\end{equation}
Note that this uses the ``left-up'' convention,
but other conventions
such as the ``right-up'' convention exist~\citep{bruns2015names}.

\subsection{Tied game types}
This taxonomy~\citep{bruns2015names}
gives compound names to games with tied payoffs, such as
``middle deadlock'' for a tie between deadlock and dilemma games:
\begin{equation}
\begin{bNiceMatrix}[first-col,first-row,hvlines]
  & C & N \\
  C & 3 & 1 \\
...
\label{eq:tied_game}
\end{equation}
These non-strict games occur on the borders of parameter space
between two game regions.
\Cref{fig:phase-diagram-B_alpha} shows that we have non-strict games
exactly when $\alpha = 1$, or $B/c = 0$
(\ie{} $\Delta \phi = \pi$).
We chose to simplify the game type nomenclature in our plots
by using a convention to break ties.
First, we put the payoff matrix in standard form
by simultaneously swapping the rows and columns
to ensure $4$ is in the leftmost column,
and the transpose of the matrix has $4$ in the uppermost row.
Then, if any entries of the matrix are equal,
we break the ties according to the following order
\begin{equation}
\begin{bNiceMatrix}[first-col,first-row,hvlines]
  & C & N \\
  C & 4 & 1 \\
...
\end{bNiceMatrix}
\end{equation}
For instance, the example in \cref{eq:tied_game}
is already in standard form,
so the tie between the $CC$ and $NN$ entries
is broken as $CC > NN$,
and we label the \cref{eq:tied_game} matrix
as a dilemma type game.
Referring back to \cref{fig:phase-diagram-B_alpha},
our convention breaks the $\alpha = 1$ ties as follows:
deadlock and compromise ties are labelled deadlock, % low lock
chicken and battle ties are labelled chicken, % mid battle
and
compromise and harmony ties are labelled compromise. % high harmony
Additionally, all of the $B/c = 0$ ties
correspond to the payoff matrix
\begin{equation}
\begin{bNiceMatrix}[first-col,first-row,hvlines]
  & C & N \\
  C & -c & -c \\
...
\end{bNiceMatrix}
\end{equation}
with cost $c = 0.1$, which we label as deadlock. % double compromise
