...
\abstract{
  ...
  of both synchronized and incoherent populations.
  ...
  Recent work has even suggested that Kuramoto models
  distributed according to the \gls{dmelanogaster} connectome
  gives rise to chimera-like states~\citep{deng2024chimera}.
  % General problem
  ...
  % Main results
  We generalize a prior application
  of evolutionary Kuramoto theory on complete graphs
  to networks with weighted, directed graphs
  and add a payoff asymmetry between communicators
  and non-communicators.
  The \gls{celegans} network exhibits far more disorder
  (order parameter $\rho \in \SIrange{40}{70}{\percent}$) than
  the well-mixed population which is virtually always synchronized
  ($\rho \sim 1$),
  and we are able to explain the types of games played by each system
  through an analysis of the payoff parameter space.
  Additionally, we find that the network is
  quite unstable with metastability index $\lambda < \num{0.01}$
  but has a relatively large chimera-like index $\xi$ of up to $\num{0.13}$.
  % General context
  Our results illustrate one potential way
  that evolutionary pressures on individual neurons
  in highly-nontrivial network topologies
  could give rise synchronization via Kuramoto-like mechanisms.
  % Broader perspective
  The observation of chimera-like states
  in these \gls{celegans} networks of simple neurons
  suggest that the brain connectivity,
  as opposed than the neurons' physical properties,
  might play key roles in the enhancement of observed chimera-like brain states.
...
% TODO: what kind of weighted graphs?
...
This model differs from \citet{antonioni2017coevolution}
by encompassing games beyond the prisoner's dilemma.
...
% TODO: Add a bit about ``major finding'' here
...
% TODO: - 1a: network of well-mixed, with blow-up to fig 1b
\begin{figure}
  \centering
...
% TODO: When playing games, how is directionality of edges handled?
% (C) -> (N) do you decompose the game into donation game?
% TODO: When describing how digraphs give payoffs (text or caption),
% specify what percent of links are unidirectional and what percent are bidirectional
...
% TODO: 81% is arbitrary; can we change it or justify it?
at least \SI{81}{\percent} of the maximum,
% TODO: Explain why we use this particular setup
...
\begin{figure}
  ...
  \end{subcaptiongroup}
  % TODO: Add bold summary to caption beginnings
  \caption{
    \textbf{
...
to the setup in \citet{tripp2022evolutionary}
...
% TODO: Can we tag data when mutations occur?
...
\begin{figure}
  \centering
  % TODO: Remove theory line for celegans
  \begin{subcaptiongroup}
...
% TODO: update figure to only show non-uniform games
% TODO: add sentence in paper or caption giving percent of time non-uniform
\begin{figure}
  \centering
  \begin{subcaptiongroup}
...
to those well-mixed, $\alpha = 0.5$ case treated in \citet{tripp2022evolutionary};
...
This is consistent with the $\alpha = 0.5$ column
...
show a small
(\SI{13}{\percent}, \SI{3}{\percent}, and \SI{1}{\percent}, respectively)
heterogeneity.
...
%\section{Conclusion}
% TODO: we will add a comprehensive discussion
% regarding our contribution and limitations, and future work
...
of \glsxtrlong{dmelanogaster}~\citep{schlegel2024whole}.
...
% TODO: new figure:
% - 3a: change to waterfall plot?
% - 3a: remove meaningless green lines connecting points
% - 3: blow-up some points from 3a to multiple timeseries plots
% - 3: increase aspect ratio of timeseries plots (wide and short)
% - 4: show additional alphas as well as unweighted+directed and undirected+weighted
% - 5 snapshots: highlight communities?
% - 5: split metastability and chimera
