...
See \cref{sec:order_graphs} of the supplemental material
...
\begin{figure*}
  ...
  \caption{
    ...
    weighted, directed \gls{celegans} connectome.
    A blue inset above the
    \protect{\subref{fig:game-type_well-mixed}}
    well-mixed panel shows the magnified upper region.
    The selection strength is $\delta=0.2$,
    the mutation rate is $\mu=\num{1E-4}$,
    the cost $c$ is \num{0.1},
    the maximum joint benefit $B_0$ is \num{0.15},
...
This connection between exponentially-slow fixation time games and chimera index
...
In the \cref{sec:analytic_comm_frac} of the supplemental material,
...
Then, We form a community by collecting all nodes $j$ with covariance-sum
at least \num{1}, \ie{} $\sum_i K_{i,j} \ge 1$.
For reference, the maximum covariance is \num{184} and the mean is \num{0.03},
so this cut-off ensures we only group nodes with high covariance-sum.
Finally, we place all the remaining nodes in a second community.
For these simulations, this produces a high-covariance-sum community
of 54 nodes and a low-covariance-sum community of 246 nodes.
