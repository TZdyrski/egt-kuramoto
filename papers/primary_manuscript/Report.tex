...
%\subsection{Outline}
In this paper,
we introduce a asymmetric evolutionary Kuramoto model
...
Additionally, we can consider the distribution of disordered
(\ie{} states other than ``all-C'' or ``all-N'')
...
between two groups with fractions between $0.5 \pm \sqrt{(N-1)/(4N\overline{d})}$,
with average degree $\overline{d}$ (\cf{} supplementary material section 3.2).
This corresponds to
\SIrange{39}{62}{\percent} (\numrange{8}{12} players in each group)
for the $N = 20$ well-mixed case ($\overline{d} = N-1$)
and
\SIrange{36}{64}{\percent} for the \gls{celegans} case ($\overline{d} \approx \num{12.36}$).
Indeed, this $\approx \SI{15}{\percent}$ range is visible
in both \cref{fig:game-type_well-mixed-only-mixed-games}
and \cref{fig:game-type_celegans-full-only-mixed-games}
as the percentage of time spent in non-synchronized games (\ie{} staghunt and concord)
as a fraction of all disordered (non ``all-C''/``all-N''), mixed games.
