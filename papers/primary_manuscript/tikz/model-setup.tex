\begin{tikzpicture}[x=0.3cm,y=0.3cm]
  \matrix{
    \begin{scope}[scale=10]
      ...
      \csvreader[head to column names]{data/processed/vertices_adj_matrix_source=well-mixed.csv}{}%
      {%
        \node[draw, fill=white, circle, inner sep=1.5] (\index) at (\x, \y) {};
      }
      \end{scope}
      \node [text width=1cm,anchor=north west] at (-12,7) {\subcaption{\label{fig:graph_well-mixed}}};
      &
      \node (center) {};
      ...
      \draw[- Latex, thick] (neuron_j.south west) -- node[midway,above,anchor=north] (arrow_bot) {$w_j$} (neuron_i.south east);
      \draw[- Latex, thick] (neuron_i.north east) -- node[midway,below,anchor=south] {$w_i$} (neuron_j.north west);

      \node [align=left,below=of arrow_bot] {Strategy $s_i$: \begin{tabular}{l} \textbf{C}: communicative \\ \textbf{N}: noncommunicative\end{tabular}
          \\[10]
      Phase difference: $\Delta \phi = \phi_j - \phi_i$};
    \node [text width=1cm,anchor=north west] at (-12,7) {\subcaption{\label{fig:player_interactions}}};
    \\
    \node {
    ...
    \end{bNiceMatrix}$};
    \node [text width=1cm,anchor=north west] at (-12,7) {\subcaption{\label{fig:payoff_matrix}}};
    &
    \\
    };
    ...
    \begin{scope}[on background layer]
      ...
      % Read edges and draw connections between nodes
        \csvreader[head to column names]{data/processed/edges_adj_matrix_source=well-mixed.csv}{}%
      {%
