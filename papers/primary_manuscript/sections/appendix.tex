...
but every node has approximately the same degree $\overline{d}$
...
to the incomplete graph with degree $\overline{d}$
is equivalent to keeping only a $\overline{d}/(N-1)$ subset
...
are scaled down by a factor of $\overline{d}/(N-1)$.
