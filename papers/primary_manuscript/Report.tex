...
of $M/[4(M-1)] = \num{0.5}$~\cite{shanahan2010metastable}
...
at $\lambda \approx \num{0.02}$ for the $\alpha = 0.5, 1$ case,
the metastability drops precipitously for other $\alpha$,
always staying below \num{0.008}.
Conversely, the chimera-like index $\chi$
indicates a high chimeric quality
with values (\numrange{0.14}{0.23})
nearly half the theoretical maximum (\num{0.5}).
...
While we did calculate the communities
based on the $\alpha = \num{0.75}$ covariances,
the high $\chi$ value for all asymmetries $\alpha$
...
chimera-like index $\chi = \num{0.23}$ observed in \cref{fig:chimera-index} for $\alpha = 0$.
In comparison, the $\alpha = 0.75$ case depicted in \cref{fig:graph}
has an slightly lower $\chi = \num{0.21}$,
consistent with the smaller synchronized
non-communicative group (red).
Finally,
\cref{fig:graph_celegans_asymmetry1} shows a snapshot for $\alpha = 1$,
for which we expect a slightly lower chimera-like index of $\chi = \num{0.20}$.
Indeed, we see that the medium-sized communicative (red) group
...
slightly lowering the chimeric quality.
...
by taking a subset
(the first \num{8E4} steps for computational ease)
...
at least \SI{80}{\percent} of the maximum covariance-sum,
\ie{} $\sum_i K_{i,j} \ge 0.8 \max_j\pab{\sum_i K_{i,j}}$.
Note that \SI{80}{\percent} is chosen as a high cutoff
to ensure we only group nodes with high covariance-sum;
for reference, this splits the population into
communities of size \num{81} and \num{219}.
...
= 1/2$ for our $M=2$~\cite{shanahan2010metastable}.
