...
\documentclass[aps,prx,preprint,superscriptaddress,linenumbers]{revtex4-2}
...
\author{\href{https://orcid.org/0000-0003-3039-172X}{Thomas Zdyrski}}
\affiliation{Department of Mathematics, Dartmouth College, Hanover, NH 03755}
\author{\href{https://orcid.org/0000-0002-0281-2868}{Scott Pauls}}
\affiliation{Department of Mathematics, Dartmouth College, Hanover, NH 03755}
\author{\href{https://orcid.org/0000-0001-8252-1990}{Feng Fu}}
\affiliation{Department of Mathematics, Dartmouth College, Hanover, NH 03755}
\affiliation{Department of Biomedical Data Science, Geisel School of Medicine at Dartmouth, Lebanon, NH 03756}

\keywords{neural synchronization, game theory, evolutionary graph theory}

%TC:ignore
\begin{abstract}
Neural synchronization is central to cognition~\citep{santos2017chimera,bansal2019cognitive}.
However, incomplete synchronization often produces chimera
states~\citep{majhi2019chimera,hizanidis2016chimera},
where coherent and incoherent dynamics coexist.
While previous studies~\citep{abrams2004chimera}
...
where interaction payoffs depend on phase alignment~\citep{tripp2022evolutionary}
...
Applying our framework to the \celegans{}
connectome~\citep{cook2019whole,towlson2013rich,yan2017network},
...
%\subsection{\celegans{} connectome}
The nematode \celegansfull{}
is a model organism in neuroscience due to
its simple brain connectome~\citep{cook2019whole}
of only \num{302} neurons.
Despite their simplicity, models of the \celegans{} brain
...
\celegans{} connectome.
In \cref{sec:methods}, we setup the model and describe the various
statistics collected.
\Cref{sec:results} details the numerical simulation results while
connecting individual neuron fitness and non-trivial brain topology
to chimera-like brain states.
Finally, \cref{sec:discussion} discusses the relevance of the results
and positions the model in the context of existing work.
...
introducing a payoff asymmetry
and
placing the player network on a directed, weighted graph.

\subsection{Game setup and payoff asymmetry}\label{sec:game_setup}
First, we generalize the payoff structure to incorporate
an asymmetry between communicators and non-communicators.
We treat each player (node) as an evolving oscillator
by discretizing its $2\pi$ phase angle into $m$
discrete phases $2 \pi j/m$ for $j \in 0, \ldots, m-1$.
Additionally, each player has a strategy $s_i$,
either communicative $C$ or non-communicative $N$.
Thus, the game's strategy space is the outer product of the $m$ phases
...
the asymmetry $\alpha \in [0,1]$ biases the payoff toward (against)
...
\subsection{Population setup on a graph}\label{sec:pop_setup}
\begin{figure*}
  \centering
  \begin{subcaptiongroup}
  ...
  \caption{
    ...
    The payoff matrix (\cf{} \protect{\cref{eq:payoff-matrix}})
    shows the reward the row-player $(C, \phi_i)$
...
The other EK model generalization we implemented
was placing players on a graph.
First, we can represent a well-mixed population
as a complete graph, as in \cref{fig:graph_well-mixed},
...
For reference, the \celegans{} connectome
has \num{38} self-loops, \num{669} bidirectional edge pairs,
and \num{2331} unpaired edges.
Finally, we interpret the \celegans{} chemical connectome's
...
Since the \celegans{} edge weights are integers,
...
chimera-like states \celegans{} models,
...
for the \celegans{} simulation with asymmetry $\alpha = \num{0.75}$.
...
\begin{figure*}
  ...
  \caption{
    ...
    ommunication frequency $f_{\text{comm}}$
...
\subsection{\celegans{} graphs}\label{sec:elegans_graph}

Here, we consider the weighted, directed hermaphroditic \celegans{}
...
\begin{figure*}
  ...
  \caption{
    \justifying
    Rise of chimera states in \celegans{} neural network.
    ...
    weighted, directed \celegans{} connectome,
    \protect{\subref{fig:comm-frac_celegans-unweighted}}
    unweighted, directed \celegans{} connectome,
    and
    \protect{\subref{fig:comm-frac_celegans-undirected}}
    weighted, undirected \celegans{} connectome.
    \protect{\subref{fig:graph}}
    The network topology for the \celegans{}
...
for the \celegans{} connectome network topologies.
All subplots use an asymmetry of $\alpha=0.75$.

We note that the behaviour
of the \subref{fig:comm-frac_celegans-full} \celegans{} case
...
of the \subref{fig:comm-frac_celegans-full} full \celegans{} case.
Conversely,
...
between the \subref{fig:comm-frac_celegans-full} full \celegans{}
case and the \cref{fig:multi-comm-frac} well-mixed case.

We can also investigate the time-evolution of the \celegans{} system
using the same parameters as the
\crefrange{fig:time-series_well-mixed_alpha-0}{fig:time-series_well-mixed_alpha-1}
well-mixed case but with the \celegans{} connectome graph.
Compared to the well-mixed case,
the \subref{fig:time-series_celegans-full} \celegans{} case
...
the \cref{fig:time-series_celegans-full} \celegans{} setup
only displays hero and staghunt games.
Finally, the \celegans{} case is less stable to mutations
...
the \celegans{} case depicts disordered states with discontinuous offsets.
...
\begin{figure*}
  ...
  \caption{
    ...
    A snapshot of the \celegans{} connectome where
    blue colors are communicative,
    red colors are non-communicative,
    and shades represent different phases $\phi$.
    The asymmetry is
...
\begin{figure*}
  ...
  \caption{
    \justifying
    Influence of payoff asymmetry on plurality game type
    played amongst all player interactions
...
In contrast, the
\subref{fig:game-type_celegans-full} \celegans{} system
...
Additionally, the \celegans{} system displays
...
Another study of the \celegans{} connectome also found
...
of the \celegans{} connectome considered.
...
chimera-like brain states in \dmelanogasterfull{}
...
a family of graphs deriving from the \celegans{} connectome.
...
the \celegans{} population remains far more heterogenous
...
such as that of
\dmelanogasterfull{}~\citep{schlegel2024whole}.
...
time-evolution of \celegans{} player strategies
...
The source code that support the findings of this article are openly
available~\citep{zdyrski2025evolutionaryCodeZenodo}.

\subsection{Data availability}
The data that support the findings of this article are openly available~\citep{zdyrski2025evolutionaryData}.
