...
\begin{tikzpicture}
    ...
    \begin{groupplot}[
      ...
      every axis plot/.append style={
        ...
        % Fill in regions
        \addplot[staghunt] fill between[of=R_T and xupper,
        ...
        \addplot[dilemma] fill between[of=R_P and R_T,
          soft clip={domain=0:1}];
    ...
    \path (group c1r1.south west) -- node[below={1.5cm}]{\pgfplotslegendfromname{CommonLegend4}} (group c1r1.south east);
		%\path (group c2r1.south west) -- node[below={1.5cm}]{\begin{tabular}{c|l}
		\node [below={1.5cm},anchor=north west] at (group c2r1.south west) {\begin{tabular}{c|l}
			Parameter & Select transitions as $\Delta \theta = 0 \to \pi$ \\
			\hline
			$B_0/c=1.5$ & chicken $\to$ dilemma $\to$ staghunt \\
			$\alpha = 1.0$ & deadlock $\to$ harmony $\to$ staghunt \\
			$\alpha = 0.75$ & hero $\to$ concord $\to$ staghunt \\
			$\alpha = 0.25$ & chicken $\to$ concord $\to$ staghunt
		\end{tabular}} (group c2r1.south east);
\end{tikzpicture}
\end{document}
