...
in the weak-selection and low-mutation-rate limits.
Since the mutation rate is low,
we only need to consider a population with only two strategies.
To calculate the steady-state communication fraction,
we will consider a population with one communicative population $E = (C, \phi_i)$
and one non-communicative population $F = (N, \phi_j)$.
...
with $\delta$ the selection strength.
...
In particular, since the Moran process changes at-most one strategy per time step,
this Markov chain has the same fixation probabilities
as an absorbing, heterogeneous one-dimensional random walk in $i$;
unlike a true random walk, this Markov chain can take a step a zero-length step,
but this doesn't affect the fixation probabilities.
Therefore, we denote $x_i$ as the probability that state $i$ will fixate
...
from state $i$ to state $j$ is determined by the Moran process as
\begin{align*}
  p_{0,0} &= 1, \\
  p_{n,n} &= 1, \\
...
\end{align*}
Furthermore, since each state $i$ can only transition
to itself or an immediate neighbor,
...
we need to derive the form of the $\gamma_k$,
