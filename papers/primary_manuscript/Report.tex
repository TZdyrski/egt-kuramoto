...
First, we represent the well-mixed population in \tripp{}
...
where game (edge) payoffs only flow to the tail players (nodes).
...
Using $\alpha = 0.5$ reproduces the symmetric case of \tripp{},
...
well-mixed setup
and the most-frequent plurality games in the \cref{fig:game-type_celegans-full}
\gls{celegans} setup
are identical,
so this discussion applies to both setups.
We note that the most-frequent plurality games
...
For the well-mixed case, the game type corresponding
...
This same analysis of graph degree explains why the
\cref{fig:game-type_celegans-full} \gls{celegans} graph
exhibits game-type heterogeneity
for $\alpha = 0,0.25,0.5$,
...
than the well-mixed case,
the average degree $\overline{d}$
is smaller (\num{12.36} \vs{} \num{19}).
Therefore, if we consider the \gls{celegans} graph
with two major populations
and assume they are spatially distributed randomly (for ease of computation),
...
as well as a spread of disordered players with varying phases/colors.
In contrast,
